...
def custom_collate_fn(batch):
    ...
    vid3_batch = torch.stack([item['vids'][2] for item in batch])

    collated_batch = {
        'vids': (vid1_batch, vid2_batch,vid3_batch),  # (vid1, vid2) の形にする
...
def test(model, net, lossfunc):
    ...
    with torch.no_grad():
        dataset = FrontProfile(opt.video_path,
                               ...
                               'test')
        ####################
        #ここでopt.val_listにしないといけないかも？
        #####################
        dataset = MultiView(opt.video_path,
                               opt.anno_path,
                               opt.train_list,
                               opt.vid_padding,
                               opt.txt_padding,
...
def combinationLoss(model, net):
    ...
    for epoch in range(opt.max_epoch):
        ...
        for i_iter, input in enumerate(loader):
            ...
            loss = alpha * contrastive_loss + beta * ctc_loss
            ...
            #ログ
            if tot_iter % opt.test_step == 0:
                val_loss, val_wer, val_cer = test(model, net, "combi")
                wandb.log({
                    "val_epoch": val_epoch,
                    "val_loss": val_loss,
                    "val_cer": val_cer,
